 ...
 */
void drawChar11x16(u_int rcol, u_int rrow, char c, 
     u_int fgColorBGR, u_int bgColorBGR) 
{
  u_int col = 0;
  u_int row = 0;
  u_int bit = 0x0001;  //0x0001
  u_char oc = c - 0x0020;//0x20  //40 best  c-
  //10, 16
  lcd_setArea(rcol, rrow, rcol + 10, rrow + 16); /*10  16 relative to requested col/row */
  while (row < 17) {           //17
    while (col < 11) {   //11           //& bit
 ...
 */
void drawString11x16(u_int col, u_int row, char *string,
		u_int fgColorBGR, u_int bgColorBGR)
{
  u_char cols = col;
  while (*string) {
